...
def proceso_analisis(ruta_img, image_file, numero_aleatorio, fecha_actual):
  ...
  ## Elimina ruido blur, mejora la luminosidad canal L.
  ...
  ## 
  # mask = mask_verde | mask_verde_2 | mask_verde_3 | mask_verde_4 | mask_deshidratado | mask_deshidratado_2 | mask_bandeja
  mask = mask_verde | mask_verde_2 | mask_verde_3 | mask_verde_4 | mask_verde_5 | mask_deshidratado
  # mask = mask_verde | mask_verde_2 | mask_verde_3 | mask_verde_4 | mask_verde_5 | mask_deshidratado | mask_deshidratado_2
  # mask = mask_verde | mask_verde_2 | mask_verde_3 | mask_verde_4 | mask_deshidratado
  ...
  # relevant_sin_brillo_gray = cv2.cvtColor(relevant_sin_brillo, cv2.COLOR_BGR2GRAY)
  ...
  for index, cnt in enumerate(contours):
    ...
    if (w > tamanio_w and h > tamanio_h and (w*h) > (tamanio_h*tamanio_w) and (w>112 != h>112)):
      #Se define el radio de las circunferencias a analizar  ## REVISAR ACA Y APLICAR EL CALCULO EN BASE ANGULO VISUAL
      area = cv2.contourArea(cnt)
  ...
  if results:
      ...
      cv2.imwrite(ruta_guardar+"resultado"+numero_aleatorio + version + fecha_actual+".jpg", img_result)

      largo = len(results)
      documento(results, largo, h, ruta_guardar, name_file, image_file, tam_y, tam_x, numero_aleatorio, fecha_actual)
