...
def export_to_pdf(data, image_file, lista_color, lista_calibre, ruta_guardar, name_file, h):
    c = canvas.Canvas(ruta_guardar+"Reporte_Arandano"+version+name_file+".pdf", pagesize=A4)
    ...
    for rows in grouper(data, max_rows_per_page):
        ...
        c.showPage()
    # print(name_file)
    c.drawImage(ruta_guardar + 'Resultado'+version+name_file, 80, h - 50, width = 490, height = 700)
    c.save()
...
def guardar_excel(data,ruta_guardar, name_file):
    import xlsxwriter
    workbook = xlsxwriter.Workbook(ruta_guardar + "Arandanos"+version+name_file+".xlsx")
...
def documento(results, largo, image_file, ruta_guardar, name_file, h):
    ...
    export_to_pdf(data,image_file,lista_color,lista_calibre, ruta_guardar, name_file, h)
