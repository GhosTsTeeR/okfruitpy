...
from prueba_algoritmos_v9_5.declarar_rutas_multiimagenes import *
...
for image in glob.glob(image_file):
    ...
    pixel_inch = 260 # 300 # 290 # 260 # 240 # 220 # 200
            ...
            if (((w*h)  > area_prom/2) and (w*h < 99900)):
                ...
                if (diferencia < valor):
                    ...
                    if (w < prom_w*1.5 and h < prom_h*1.5 and y-marco > 0 and y+h+marco > 0 and x-marco > 0 and x+w+marco > 0):
                        ...
                        # cv2.imwrite(ruta_guardar+"roi_"+str(numero_fruto) + version + name_file, cv2.cvtColor(roi, cv2.COLOR_RGB2BGR))
