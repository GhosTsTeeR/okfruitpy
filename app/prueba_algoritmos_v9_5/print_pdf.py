...
def export_to_pdf(data,image_file,lista_color,lista_calibre,lista_pedicelo, name_file, h, ruta_guardar, tam_y, tam_x):
    c = canvas.Canvas(ruta_guardar + "Reporte_Cerezas" + version + name_file+".pdf", pagesize=A4)
    ...
    c.drawImage(ruta_guardar + 'Resultado' + version + name_file, 20, 20, width = sp_width, height = sp_heigh)
    c.save()
...
def guardar_excel(data,ruta_guardar, name_file):
    import xlsxwriter
    workbook = xlsxwriter.Workbook(ruta_guardar + "Cerezas" + version + name_file+".xlsx")
...
def documento(results, largo, h, ruta_guardar, name_file, image_file, tam_y, tam_x):
    ...
    export_to_pdf(data,image_file,lista_color,lista_calibre,lista_pedicelo, name_file, h, ruta_guardar, tam_y, tam_x)
    guardar_excel(data, ruta_guardar, name_file)